...
>(
  (
    ...
    // Move cookie operation to a separate effect to avoid blocking the toggle
    React.useEffect(() => {
    ...
    // Simplified toggle without useCallback since it's already memoized in context
    const toggleSidebar = () => {
      if (isMobile) {
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      'pl-2 flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden',

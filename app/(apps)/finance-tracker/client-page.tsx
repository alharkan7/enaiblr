...
export default function FinanceTrackerClient({ initialSession }: { initialSession: Session }) {
  // Auth state verification
  const { status: authStatus } = useSession();
  const router = useRouter();

  // If the SessionProvider detects the user logged out, redirect them immediately.
  // This bypasses the Next.js router cache of the RSC payload!
  useEffect(() => {
    if (authStatus === 'unauthenticated') {
      router.push('/login?callbackUrl=/finance-tracker');
    }
  }, [authStatus, router]);

  // Use session passed from server component
  ...
  const fetchUserCategories = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        let expCats = data.expense_categories || []
        let incCats = data.income_categories || []
        
        if (typeof expCats === 'string') {
          try { expCats = JSON.parse(expCats) } catch (e) { expCats = [] }
        }
        if (typeof incCats === 'string') {
          try { incCats = JSON.parse(incCats) } catch (e) { incCats = [] }
        }
  ...
  // Prevent hydration mismatch by returning skeleton during SSR
  if (!isMounted) {
  ...
  return (
    <div className="w-full relative overflow-hidden" style={{ height: 'calc(var(--vh, 1vh) * 100)' }}>
      ...
      <div className="relative z-10 h-full w-full max-w-sm mx-auto flex flex-col">
        ...
        {/* Main Content */}
        <div className="flex-1 bg-background rounded-t-3xl px-4 pt-4 pb-0 w-full overflow-hidden flex flex-col items-center relative border-t">
        ...
        <div className="flex gap-2 p-3 mb-0 bg-background w-full flex-shrink-0 rounded-b-lg border-t">
          <Button
            variant="secondary"
            className="rounded-full flex-1 h-8 text-xs border border-border shadow-none hover:shadow-none hover:translate-x-0 hover:translate-y-0 bg-transparent"
          ...
          <Drawer open={isDrawerOpen} onOpenChange={(open) => {
            ...
            <DrawerTrigger asChild>
              <Button variant="secondary" className="rounded-full flex-1 h-8 text-xs border border-border shadow-none hover:shadow-none hover:translate-x-0 hover:translate-y-0 bg-transparent">
        ...
        <Dialog open={isBudgetAlertOpen} onOpenChange={setIsBudgetAlertOpen}>
          <DialogContent className="w-full max-w-sm mx-auto rounded-2xl">
            <DialogHeader>
              ...
              <div className="text-sm space-y-2 text-center text-muted-foreground">
                <div className="text-muted-foreground">Pengeluaranmu melebihi anggaran sebesar:</div>
